...
const Search = () => {
  ...
  return (
    <div className='min-h-[32px] h-[53px] mb-3 flex items-center'>
        <label className='h-[43px] rounded-full bg-[#202327] w-full relative border border-transparent focus-within:bg-black focus-within:border-[#1d9bf0]'>
        ...
        <input type="text" className = "w-full h-full bg-transparent rounded-full outline-none pl-[56px] text-[15px]" placeholder = "Ara" onChange ={(e)=> setQuery(e.target.value)} value = {query}/>
        {
            query && (
                <button type="button" className="w-[22px] h-[22px] rounded-full bg-[#1d9bf0] flex items-center justify-center text-black min-w-[22px] absolute top-1/2 -translate-y-1/2 right-3 " onClick={()=>setQuery("") }>
